...
describe("render > ", () => {
  ...
  describe("두 번째 렌더링 테스트", () => {
    test("다중 계층 태그를 렌더링할 수 있다.", () => {
      const App = jsx(
        "div",
        null,
        jsx(
      ...
      expect($root.innerHTML).toBe(
        `<div><div class="fw"><button class="fb" type="button"><span>1</span></button><button type="button"><span>2</span><img src="http://example.com"></button></div><p id="_p">두 번째 문단</p><p>세 번째 문단</p></div>`
      );
    });
  });
});
